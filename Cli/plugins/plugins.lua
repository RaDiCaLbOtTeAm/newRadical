...
local function list_plugins(only_enabled, msg)
  ...
  text = "\n`💥سیستم ربات (مگا پلاس) با موفقیت ریست شد`\n`کانال ما:`"..msg_caption
...
local Clang = redis:get(Chash)
  ...
  if (matches[1]:lower() == 'reset' and not Clang) or (matches[1]:lower() == 'ریست' and Clang) and is_sudo(msg) then --after changed to moderator mode, set only sudo
    return reload_plugins(true, msg)
  end
end
